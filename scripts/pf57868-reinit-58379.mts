/**
 * PF-58379 reinit — Accrued Interest Kenya N/A reconfirm (mouse-only).
 * Already Jira DONE — only keep Done if Accrued Interest absent on cNwNb with proof.
 */
...
const OUT = "E:/QA OUTPUTS/PF-57868-reinit/proof/PF-58379";
...
async function main() {
  console.log("=== REINIT PF-58379 Accrued Interest N/A ===");
  ...
  for (let i = 0; i < 40; i++) {
    const t = await body(page);
    if (/Welcome to Lending|Transaction Management|Settings/i.test(t) && !/personalization is in progress/i.test(t)) break;
    await sleep(1200);
  }
  ...
  if (await search.count()) {
    ...
    await search.fill("Accrued Interest");
    await sleep(1200);
    await shot(page, "search-accrued");
  }
  const hitAccrued = await mouseText(page, /Accrued Interest/i, "accrued-hit");
  await sleep(2000);
  await shot(page, "after-accrued-search");

  // also search Settings tiles
  ...
  await mouseText(page, /Accrued/i, "settings-accrued");
  await sleep(1500);
  const t = await body(page);
  const shotA = await shot(page, "assert-na");
  const found = /Accrued Interest/i.test(t) && hitAccrued;
  // N/A if search yields no navigable Accrued Interest feature
  const na = !found;

  const findings = [
    {
      bug: "N/A-Kenya",
      claim: "Accrued Interest Calculation available on Kenya Lending",
      verdict: na ? "N/A_CONFIRMED" : "FOUND_ON_BUILD",
      notes: na
        ? "Sidebar/Settings search for Accrued Interest — no feature entry on cNwNb Kenya Lending this pass"
        : "Accrued Interest control found — re-open story / not N/A",
      shot: shotA,
    },
  ];
  ...
  fs.writeFileSync("E:/QA OUTPUTS/PF-57868-reinit/tracker/PF-58379.json", JSON.stringify(summary, null, 2));
  console.log("REINIT_58379_DONE", JSON.stringify(summary, null, 2));
}
